...
// reports.php â€“ OAS-raporttisivu (parametrien valinta)
...
// Ei nÃ¤ytetÃ¤ tuloksia ennenkuin valinnat on olemassa ja Painetaan nÃ¤ppÃ¤intÃ¤
$isRun = (isset($_GET['run']) && $_GET['run'] === '1');

// Placeholder tyhjÃ¤lle account-arvolle
const EMPTY_LABEL = '(tyhjÃ¤)';
...
// Lomakekenttien oletusarvot (kÃ¤ytetÃ¤Ã¤n myÃ¶s auditissa)
...
// Kirjaa aina sivulle tulo (GET), jotta admin nÃ¤kee kÃ¤ytÃ¶n
...
// Jos kÃ¤yttÃ¤jÃ¤ vaihteli valintoja, mutta ei vielÃ¤ ajanut raporttia -> parametrimuutos
...
// Add this section after line 140 (after range error check) in reports.php
...
if (isset($_GET['from_ajax']) && $_GET['from_ajax'] === '1' && isset($_SESSION['report_results'])) {
    ...
    $paramsMatch = (
        ...
        (time() - $savedResults['timestamp']) < 300 // 5 minutes
    );

    if ($paramsMatch) {
        $useSessionData = true;
        // Use pre-calculated values
        $reach3pSum = $savedResults['total_reach'];
        $avgViewersSum = $savedResults['total_avg_viewers'];
        $playedCount = $savedResults['processed_count'];
        $matchedSpots = $savedResults['processed_count'];
        ...
        audit_log('report_used_session_data', [
            ...
            'from' => $effFrom,
            'to' => $effTo,
            'age_seconds' => time() - $savedResults['timestamp'],
        ]);
    }
...
// Only do the heavy calculation if we're NOT using session data
if ($isRun && !$rangeError && $campaign !== '' && $effFrom && $effTo && !$useSessionData) {
    // ... existing calculation code stays here ...
    // (lines 143-287 remain unchanged)
}

$minISO = $maxISO = null;
$minDisp = $maxDisp = 'â€”';

// --- Jos sekÃ¤ account ettÃ¤ campaign on valittu, yliaja pÃ¤ivÃ¤t OATV-taulun perusteella ---
...
if ($effFrom && $effTo && $effFrom > $effTo) {
    $rangeError = 'AikavÃ¤li on virheellinen: aloituspÃ¤ivÃ¤ on suurempi kuin lopetuspÃ¤ivÃ¤.';
}
if ($isRun && $rangeError !== '') {
...
";
    ...
    try {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            // Calculate TRP for this duration
...
// Sama kampanjafiltteri kuin pÃ¤Ã¤raportissa
$campaignFilter = ($campaign === EMPTY_LABEL)
    ? "(o.artist IS NULL OR TRIM(o.artist) = '')"
    : "TRIM(o.artist) = :campaign";

// Sama pÃ¤ivÃ¤mÃ¤Ã¤rÃ¤ OATV:lle (sis. %e.%c.%Y)
$o_date = "
    COALESCE(
        CASE WHEN o.`date` REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' THEN STR_TO_DATE(o.`date`, '%Y-%m-%d') END,
        STR_TO_DATE(o.`date`, '%d.%m.%Y'),
...
if ($isRun && $debugOn && !$rangeError && $campaign !== '' && $effFrom && $effTo) {
    ...
    // 1) Kuinka monta OATV-riviÃ¤ syÃ¶tetÃ¤Ã¤n lÃ¤pi (SAMA rajaus kuin pÃ¤Ã¤raportissa)
    $sqlIn = "
        SELECT COUNT(*)
        FROM oatv o
        WHERE $campaignFilter
    ...
    // 2) Varsinainen debug-listaus â€“ TÃ„SMÃ„LLEEN sama parsinta & join kuin pÃ¤Ã¤raportissa
    $sqlDbg = "
    SELECT
      o.id AS oatv_id,
      ...
      -- 3+ ja avg_viewers samasta edellisestÃ¤ ohjelmasta
      (
        SELECT r.over60_on_channel
        FROM overnightreport r
        WHERE
...
";
    ...
    try {
        ...
        // Aiemmin tÃ¤ssÃ¤ laskettiin $avgViewersSumFromDebug ja ylikirjoitettiin
        // pÃ¤Ã¤laskenta, mikÃ¤ aiheutti eron. Nyt kÃ¤ytetÃ¤Ã¤n aina pÃ¤Ã¤laskentaa.

    } catch (Throwable $e) {
        $debugRows = [];
        if ($isAdmin && $debugOn) {
            echo '<div class="error-box">Debug-kysely epÃ¤onnistui: ' . h($e->getMessage()) . '</div>';
        }
    }
}
...
// --- Spotin pituudet (kaikki, yleisimmÃ¤stÃ¤ harvinaisimpaan) ---
...
if ($isRun && !$rangeError && $campaign !== '' && $effFrom && $effTo) {
    ...
    // Tulkitaan pituus: jos o.duration on sekunteina, CAST riittÃ¤Ã¤.
...
if (!empty($spotDurations)) {
    ...
    // fallback: nÃ¤ytÃ¤ aiempi oletus tai viiva
    // $spotLengthDisplay = '00:00:20';
}

// --- KÃ¤yttÃ¤jÃ¤n muokattavat kentÃ¤t (sÃ¤ilyvÃ¤t GET:ssÃ¤) ---
$soldSpots = isset($_GET['sold_spots']) ? trim($_GET['sold_spots']) : '';
$soldTrp = isset($_GET['sold_trp']) ? trim($_GET['sold_trp']) : '';

// --- TRP (35â€“64): Kerroin * Tavoitettu yleisÃ¶ (3+) ---
$populationN = max(0, (int) $population);
...
<form id="filtersForm" method="get" action="">
    ...
    <div class="panel">
        ...
        <div class="row">
            ...
            <div>
                <select name="campaign" <?= $selectedAccount === '' ? 'disabled' : '' ?>>
                    <option value="">â€” <?= $selectedAccount === '' ? 'Valitse ensin mainostaja' : 'Kaikki kampanjat' ?>
                        â€”</option>
        ...
        <?php if ($selectedAccount !== '' && $selectedCampaign !== ''): ?>
            <div class="subhead">Tiedolla lÃ¶ytyi kampanja, joka onâ€¦</div>
            <div class="two">
                <div class="row">
                    <div class="label">â€¦aloitettu:</div>
                    <div><?= h($minDisp) ?></div>
                </div>
                <div class="row">
                    <div class="label">â€¦lopetettu:</div>
                    <div><?= h($maxDisp) ?></div>
                </div>
            </div>
            ...
            <div class="two">
                <div class="row">
                    <div class="label">Valitse uusi aloituspÃ¤ivÃ¤:</div>
                    <div><input type="date" name="start" value="<?= h($startOverride) ?>"></div>
                </div>
                <div class="row">
                    <div class="label">Valitse uusi lopetuspÃ¤ivÃ¤:</div>
        ...
        <div class="row">
            <div class="label">Suomen vÃ¤kiluku:</div>
        ...
        <div class="row">
            ...
            <div>
                <label style="display:inline-flex;gap:8px;align-items:center;">
                    ...
                    <span>NÃ¤ytÃ¤ kaikki spotin osumat (debug-modal)</span>
                </label>
            </div>
        </div>
    <?php endif; ?>
    ...
    <div class="controls">
        <div>
            ...
            <button type="button" onclick="window.location.href='reports.php'">TyhjennÃ¤</button>
        </div>
...
if ($debugOn && $effFrom && $effTo && $campaign !== '') {
    ...
    try {
        ...
        if ($isAdmin && $debugOn) {
            echo '<div class="error-box">Probe epÃ¤onnistui: ' . h($e->getMessage()) . '</div>';
            audit_log('report_query_failed', [
                'page' => 'reports.php',
                'where' => 'probe_spot_dt',
                'error' => $e->getMessage(),
...
<?php if ($debugOn): ?>
    <div style="margin:8px 0;color:var(--muted);">
        OATV sisÃ¤Ã¤n: <?= (int) $dbg_oatv_in ?> riviÃ¤ Â· JOIN-osumia: <?= (int) $dbg_join_hit ?> riviÃ¤
    </div>
    ...
    <div style="display:flex;justify-content:flex-end;gap:10px;margin:8px 0;">
        <button type="button" class="btn" id="btnOpenDebug" <?= empty($debugRows) ? 'disabled' : '' ?>>
            NÃ¤ytÃ¤ debug-rivit (<?= count($debugRows) ?>)
        </button>
    </div>
    <?php if ($isRun && !$rangeError && $campaign !== '' && array_sum($diagCounts) === 0): {
        ...
        <div class="error-box">
            Valittu kampanjanimi â€œ<?= h($campaign) ?>â€ ei lÃ¶ydy OATV:stÃ¤
            (artist/title) valitulla aikavÃ¤lillÃ¤. Kokeile hakua vÃ¤ljemmin
            (LIKE on jo kÃ¤ytÃ¶ssÃ¤) tai varmista, kÃ¤ytetÃ¤Ã¤nkÃ¶ OATV:ssÃ¤ eri nimeÃ¤.
        </div>
    <?php endif; ?>
...
<?php if ($isRun && !$rangeError): ?>
    <!-- Taulukko on oma pikalomakkeensa, jotta muokattavat kentÃ¤t saa talteen ilman ylÃ¤osan valintojen katoamista -->
    <form method="get" action="">
        ...
        <div class="report-summary" style="max-width:1100px;margin-top:18px;">
            ...
            <!-- LeveÃ¤ taulukko: nÃ¤kyy desktopilla / isolla nÃ¤ytÃ¶llÃ¤, ja mobiilissa sivuttaisvieritys toimii -->
            <div class="h-scroll">
                <table class="table table-bordered" style="width:100%; border-collapse:collapse;">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><strong>KeskikatsojamÃ¤Ã¤rÃ¤ (summa)</strong></td>
                        ...
                        <?php if (!empty($durationStats)): ?>
                            <tr>
                                ...
                                <td><strong>Kokonaiskatsojamäärä</strong></td>
                                <td><strong>Keskikatsojamäärä</strong></td>
                            </tr>
                        ...
                        <?php endif; ?>
                            ...
                            <td><strong>TRP (35â€“64)</strong></td>
            ...
            <!-- Mobiilikortit: nÃ¤kyy vain alle 640px -->
            <div class="report-cards">
                ...
                <div class="report-card">
                    ...
                    <div class="row">
                        <!--                        <div class="label">Tavoitettu yleisÃ¶ (3+)</div>
                        <div class="value"><?= number_format((int) $reach3pSum, 0, ',', ' ') ?>
                        </div> -->
                        <div class="row">
                            <div class="label">KeskikatsojamÃ¤Ã¤rÃ¤ (summa)</div>
                            <div class="value"><?= number_format($avgViewersSum, 0, ',', ' ') ?></div>
                        </div>
                        <!--                        <div class="row">
                            <div class="label">TRP (35â€“64)</div>
                            <div class="value"><?= number_format($trp3564_pct_3p, 2, ',', ' ') ?> %</div>
                        </div> -->
                        <div class="row">
                            <div class="label">TRP (35â€“64), avg_viewers</div>
                ...
                <div class="report-card">
                    <div class="row">
                        <div class="label" style="font-size: 14px; font-weight: 700; color: var(--text);">Spotin pituuskohtaiset tilastot</div>
                    </div>
                </div>
                ...
                <div class="report-card">
                    ...
                    <div class="row">
                        <div class="label">Kokonaiskatsojamäärä</div>
                        <div class="value"><?= number_format($stats['avg_viewers_sum'], 0, ',', ' ') ?></div>
                    </div>
                    <div class="row">
                        <div class="label">Keskikatsojamäärä</div>
        ...
        <!-- <div class="controls" style="justify-content:flex-end;max-width:1100px;">
            <button type="submit" class="primary">PÃ¤ivitÃ¤ nÃ¤mÃ¤ kentÃ¤t</button>
        </div> -->
    </form>

    <?php
    // --- OATV peek (admin debug) ---
// NÃ¤yttÃ¤Ã¤ 20 viimeisintÃ¤ OATV-riviÃ¤ valitulta aikavÃ¤liltÃ¤,
// jotta nÃ¤et millÃ¤ nimillÃ¤ kampanja oikeasti esiintyy.
    $peekRows = [];
        ...
        <div id="debugModal" class="modal" aria-hidden="true">
            ...
            <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="dbgTitle">
                <div class="modal-header">
                    <h3 id="dbgTitle">Debug â€“ spotin kaikki osumat</h3>
                    <button type="button" class="modal-close" id="btnCloseDebug" aria-label="Sulje">Ã—</button>
                </div>
                <div class="modal-body">
                    ...
                    <?php else: ?>
                        ...
                        <p style="margin-top:8px;color:var(--muted);">
                            NÃ¤ytetÃ¤Ã¤n enintÃ¤Ã¤n 5000 riviÃ¤. Suodata tarkemmin, jos tarvitset tÃ¤ydellisen listan.
                        </p>
                    <?php endif; ?>
                </div>
            </div>
