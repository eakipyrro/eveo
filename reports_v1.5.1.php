...
if ($isRun && !$rangeError && $campaign !== '' && $effFrom && $effTo) {
    ...
    WHERE
        ...
        -- vain ohjelmat, jotka alkavat ennen tai samaan aikaan kuin spotti
        AND $or_start <= x.spot_dt
        -- rajaus päivämääriin kuten ennenkin
        AND DATE($or_start) BETWEEN :dfrom AND :dto
        -- tänne kannattaa lisätä kanavakentän rajaus jos sellainen on:
        -- AND r.channel = x.channel  tms.
    ORDER BY
        -- 1) ohjelmat, joiden sisään spotti osuu, ensin:
        ($or_end > x.spot_dt) DESC,
        -- 2) sitten uusin ohjelma ensin
        $or_start DESC
    LIMIT 1
...
,
    ...
    WHERE
        ...
        AND $or_start <= x.spot_dt
        AND DATE($or_start) BETWEEN :dfrom AND :dto
        -- tarvittaessa sama kanavakriteeri kuin yllä
...
";
    ...
    try {
        ...
        foreach ($debugRows as $r) {
            ...
            // Normalisoi arvo (poista välilyönnit, muuta , → .)
            $v = trim((string) $r['avg_viewers']);
            if ($v === '') {
                continue;
            }

            // Vaihda desimaalipilkku pisteeksi
            $v = str_replace(',', '.', $v);
            ...
            // Lisää summaan
            $avgViewersSumFromDebug += $num;
        }

        // Jos debug on päällä ja rivejä löytyi → käytä debug-summaa
