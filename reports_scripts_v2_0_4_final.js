// reports_scripts_v2.js - Reports page functionality with duration statistics support
...
(() => {
    ...
    async function calculateReportAjax(e) {
        ...
        try {
            ...
            // Step 1: Get count (5%)
            progressText.innerHTML = '<span>Lasketaan rivejÃ¤...</span>';
            ...
            if (total === 0) {
                alert('Ei spotteja valitulla aikavÃ¤lillÃ¤');
                return;
            }
            ...
            let durationData = {}; // Collect duration-specific data
            ...
            for (let offset = 0; offset < total; offset += chunkSize) {
                ...
                // Merge duration data
                if (chunkData.durationData) {
                    for (const [durSec, stats] of Object.entries(chunkData.durationData)) {
                        if (!durationData[durSec]) {
                            durationData[durSec] = {
                                count: 0,
                                matched: 0,
                                reach: 0,
                        ...
                        durationData[durSec].count += stats.count;
                        durationData[durSec].matched += stats.matched;
                        durationData[durSec].reach += stats.reach;
                        durationData[durSec].avg_viewers_sum += stats.avg_viewers_sum;
                    }
                }
                ...
                progressText.innerHTML = `<span>KÃ¤sitelty ${processedCount.toLocaleString('fi-FI')} / ${total.toLocaleString('fi-FI')} spottia...</span>`;
            }
            ...
            // Calculate avg_viewers_avg for each duration
            for (const [durSec, stats] of Object.entries(durationData)) {
                if (stats.count > 0) {
                    stats.avg_viewers_avg = stats.avg_viewers_sum / stats.count;
                } else {
                    stats.avg_viewers_avg = 0;
                }
            }
            ...
            params.set('duration_data', JSON.stringify(durationData));

            const saveResp = await fetch('reports_save.php?' + params.toString());
            const saveData = await saveResp.json();

            if (!saveData.success) {
                throw new Error('Tulosten tallennus epÃ¤onnistui');
            }
            ...
            progressText.innerHTML = '<span>SiirrytÃ¤Ã¤n raporttiin...</span>';
            await new Promise(r => setTimeout(r, 200));
