...
if ($isRun && $debugOn && !$rangeError && $campaign !== '' && $effFrom && $effTo) {
    ...
    SELECT
      ...
      -- 3+ ja avg_viewers samasta edellisestä ohjelmasta
           (
        SELECT r.over60_on_channel
        FROM overnightreport r
        WHERE
...
";
    ...
    try {
        ...
        // --- Keskikatsojamäärän summa debug-riveistä ---
        $avgViewersSumFromDebug = 0.0;

        foreach ($debugRows as $r) {
            if (!isset($r['avg_viewers'])) {
                continue;
            }

            // Ota raakaarvo
            $v = trim((string) $r['avg_viewers']);
            if ($v === '') {
                continue;
            }

            // Poista mahdolliset välilyönnit tuhaterottimena
            $v = str_replace(["\u{00A0}", ' '], '', $v);

            // Desimaalipilkku -> piste
            $v = str_replace(',', '.', $v);

            // Vain jos näyttää numerolta, lisätään summaan
            if (is_numeric($v)) {
                $avgViewersSumFromDebug += (float) $v;
            }
        }

        // Jos debug on päällä ja rivejä löytyi → käytä debug-riveistä laskettua summaa
        if ($debugOn && !empty($debugRows)) {
            $avgViewersSum = $avgViewersSumFromDebug;
        }
    ...
    (() => {
        ...
        async function calculateReportAjax(e) {
            ...
            try {
                ...
                // Step 1: Get count
                progressText.textContent = 'Lasketaan rivejä...';
                ...
                for (let offset = 0; offset < total; offset += chunkSize) {
                    ...
                    // Update progress
                    const percentComplete = Math.min(95, Math.floor((processedCount / total) * 90) + 5);
                    progressFill.style.width = percentComplete + '%';
                    progressText.textContent = `Käsitelty ${processedCount} / ${total} spottia...`;
                }

                // Step 3: Finalize - redirect with results
                progressFill.style.width = '100%';
                progressText.textContent = 'Viimeistellään...';

                // Add processed data to form and submit
                params.set('run', '1');
                params.set('ajax_processed', '1');
        ...
        // Debug modal handlers (keep existing)
        (() => {
